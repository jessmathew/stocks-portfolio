...
stockNamesWithHighestSharpeRatios <- sharprRatiosWithStockIndex %>% arrange(desc(SharpeRatio)) %>% slice(1:50)
...
stocksToUseForModeling <- data.frame(subset(assetsDataForCurrentTimePeriod, select=namesOfLowlyCorrelatedStocks[,1]))
stocksToUseForModelingWithDateAppended <- data.frame(assetsDataForCurrentTimePeriod[1], stocksToUseForModeling)
stocksToUseForModelingWithDateAppendedTS <- ts(stocksToUseForModelingWithDateAppended)
stocksToUseForModelingXTS <- xts(stocksToUseForModelingWithDateAppended[,-1], order.by=stocksToUseForModelingWithDateAppended[,1])

sumPortfolioLogReturns <- sum(stocksToUseForModeling)
indexes <- seq(1:ncol(stocksToUseForModeling))

portfolioWeights <- data.frame(Stock=character(), Weight= as.double(double()), stringsAsFactors=FALSE)
datalist = list()
for(stockIndex in indexes)
{
  ...
  forecastARIMA <- forecast(modelARIMA, h=5)
  
  meandf <- as.data.frame(forecastARIMA$mean)
  colnames(meandf)[colnames(meandf)=="x"] <- columnName
  # rowTest <- mean[1,]
  # transposeRow <- t(rowTest)
  
  datalist[[stockIndex]] <- meandf
  # print(rowTest)
  # print(transposeRow)
}

forecastedDf <- dplyr::bind_cols(datalist)
forecastedDf <- as.matrix.data.frame(forecastedDf)
...
# {
#   currentStock <- stocksToUseForModeling[stockIndex]
#   sumStockLogReturns <- sum(currentStock)
#   columnName <- colnames(currentStock)
# 
#   stockWeight <- sumStockLogReturns/sumPortfolioLogReturns
#   portfolioWeights[nrow(portfolioWeights)+1, ] <- c(columnName, stockWeight)
# }

tickers <- colnames(stocksToUseForModeling)
# weights <- t(portfolioWeights[2])
...
portf <- add.constraint(portf, type ="box", min=.01, max=.95)
...
multipledMatrix <- as.matrix(forecastedDf) %*% as.matrix(weightsForPortfolio)

necessarySumForThisRollingWindow <- colSums(multipledMatrix)
necessarySumForThisRollingWindow
